...
let topSpace: CGFloat = kNavBarHeight
...
open class LCSlideMenu: UIView {
    ...
    /// delegate
    var delegate: LCSlideMenuDelegate?
    ...
    public var getWidth: CGFloat {
        get {
            return self.itemsLabel[self.itemsLabel.count - 1].frame.maxX
        }
    }
    
    /// 标题字体
    public var itemFont: UIFont = UIFont.boldSystemFont(ofSize: 14) {
    ...
    /// 圆圈指示器的颜色
    public var circleIndicatorColor: CGColor = UIColor.white.cgColor {
    ...
    /// 未选中颜色
    public var unSelectedColor: UIColor = UIColor.hexString("#666666") {
    ...
    public init(frame: CGRect,titles: [String],targetVC:UIViewController ,childControllers: [UIViewController]) {
...
extension LCSlideMenu {
    ...
    private func configMainScrollView() {
        ...
        if mainScrollView.superview == nil {
            mainScrollView.frame = CGRect(x: 0, y: topSpace, width: kScreenWidth, height: kScreenHeight - topSpace)
            if((targetVC) != nil) {
                targetVC?.view .addSubview(mainScrollView)
            ...
            mainScrollView.backgroundColor = kMainBgColor
    ...
    private func configChildControllers() {
        ...
        // 添加第一个vc 到 `mainScrollView` 中
        let vc = controllers[0]
        vc.view.frame = CGRect(x: 0, y: 0, width: mainScrollView.bounds.width, height: kScreenHeight - topSpace)
    ...
    private func configTabScrollView() {
        ...
        for (index,title) in titles.enumerated() {
            ...
            // 计算title长度
            // 根据文字来计算宽度
            let size = (title as NSString).size(withAttributes: [NSAttributedString.Key.font : itemFont])
            ...
            circleIndicator.alpha = (index == itemSelectedIndex && indicatorType == .circle) ? 1 : 0
...
//        changeItemTitle(0, old: 1)
//        changeIndicatorViewPosition(0, old: 1)
...
//        }
    ...
    @objc fileprivate func labelClicked(_ gesture: UITapGestureRecognizer) {
        ...
        // 2.获取点击的lable
        let currentIndex = itemsLabel.firstIndex(of: currentLabel)!
...
//        resetTabScrollViewContentOffset(currentLabel)
        ...
        resetMainScrollViewContentOffset(currentIndex)
        itemSelectedIndex = currentIndex
        
        if let delegate = delegate {
            delegate.didSelectedTitleWithIndex(index: currentIndex)
        }
    }
    ...
    ///   - current: 当前标题索引
    ///   - old: 之前标题索引
    func changeItemTitle(_ current: Int, old: Int) {
        itemsLabel[old].textColor = unSelectedColor
        itemsLabel[current].textColor = selectedColor
        
        if titleStyle == .transfrom {   // 放大
            UIView.animate(withDuration: 0.25, animations: {
                
                self.itemsLabel[old].transform = CGAffineTransform.identity
                self.itemsLabel[current].transform = CGAffineTransform(scaleX: 1.4, y: 1.4)
//                self.itemsLabel[old].font = self.itemFont
//                self.itemsLabel[current].font = UIFont.boldSystemFont(ofSize: 18)

            })
        }
        changeIndicatorViewPosition(current, old: old)
    ...
    private func changeIndicatorViewPosition(_ current: Int, old: Int) {
        ...
        // 获取之前label的尺寸
        let frame = itemsLabel[current].frame
    ...
    private func changeCircleIndicatorPosition(_ current: Int, old: Int) {
        ...
        UIView.animate(withDuration: 0.25) {
            currentV.alpha = 1
            oldV.alpha = 0
            currentV.transform = CGAffineTransform(scaleX: 1.2, y: 1.2)
            oldV.transform = .identity
        }
    }
    ...
    private func updateTitleStyle(_ progress:CGFloat) {
        ...
        default:       // .transfrom:放大
            if progress <= 0.5 {    // 如果进度 < 0.5
                ...
                UIView.animate(withDuration: 0.25, animations: {
                    leftItem.transform = CGAffineTransform(scaleX: 1.4, y: 1.4)
                ...
                UIView.animate(withDuration: 0.25, animations: {
                    ...
                    rightItem.transform = CGAffineTransform(scaleX: 1.4, y: 1.4)
                })
            }
            break
        }
    ...
    private func resetTabScrollViewContentOffset(_ item: UILabel) {
        ...
        //item中心点超过最高滚动范围时
        if tabScrollView.contentSize.width - itemCenterX < scrollHalfWidth &&  tabScrollView.contentSize.width > self.frame.width {
    ...
    private func handleNormalIndicatorType(_ offsetX: CGFloat) {
        if offsetX <= 0 {
            ...
            rightIndex =  (leftIndex + 1) >= (itemsView.count - 1) ? (itemsView.count-1) :  (leftIndex + 1)
...
//        updateTitleStyle(relativeLacation)
        ...
        if let _ = newVc.view.superview {
            ...
            newVc.view.frame = CGRect(x: CGFloat(currentPage) * mainScrollView.bounds.width , y: 0, width: mainScrollView.bounds.width, height: kScreenHeight - topSpace)
            mainScrollView.addSubview(newVc.view)
        }
    }
